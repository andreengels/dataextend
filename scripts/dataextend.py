...
# -*- coding: utf-8 -*-
"""
Script to add properties, identifiers and sources to Wikidata items.

Usage: dataextend <item> [<property>[+*]] [args]

In the basic usage, where no property is specified, item is the Q-number of the
item to work on.

If a property (P-number, or the special value 'Wiki' or 'Data') is specified,
only the data from that identifier are added. With a '+' after it, work starts
on that identifier, then goes on to identifiers after that (including new
identifiers added while working on those identifiers). With a '*' after it,
the identifier itself is skipped, but those coming after it (not those coming
before it) are included.

There is currently one argument defined:
    --ask   If this is supplied, the bot will after each external link has been
            handled, show which changes it intends to make, and ask for
            permission.
    --force This is the reverse of --ask: If the code has been changed so that
            --ask is always on (by change FORCE = True to FORCE = False below),
            this returns to the default behaviour where all changes are made
            autonomically.

The bot will load the corresponding pages for these identifiers, and try to
the meaning of that string for the specified type of thing (for example
'city' or 'gender'). If you want to use it, but not save it (which can happen
if the string specifies a certain value now, but might show another value
elsewhere, or if it is so specific that you're pretty sure it won't occur
a second time), you can provide the Q-number with X rather than Q. If you do
not want to use the string, you can just hit enter, or give the special value
'XXX' which means that it will be skipped in each subsequent run as well.

After an identifier has been worked on, there might be a list of names that has
been found, in lc:name format, where lc is a language code. You can accept all
suggested names (answer Y), none (answer N) or ask to get asked for each name
separately (answer S), the latter being the default if you do not fill in
anything.

After all identifiers have been worked on, possible descriptions in various
languages are presented, and you get to choose one. The default is here 0,
which always is the current description for that language. Finally, for a
number of identifiers text is shown that usually gives parts of the description
that are hard to parse automatically, so you can see if there any additional
pieces of data that can be added.

It is advisable to (re)load the item page that the bot has been working on
in the browser afterward, to correct any mistakes it has made, or cases
where a more precise and less precise value have both been included.
"""
...
class DataExtendBot:
    ...
    def label(self, title):
        ...
        try:
            ...
            labels = {}
        for lang in ['en', 'nl', 'de', 'fr', 'es', 'it', 'af', 'nds', 'li', 'vls', 'zea', 'fy', 'no', 'sv', 'da', 'pt',
                     'ro', 'pl', 'cs',
                     'sk', 'hr', 'et', 'fi', 'lt', 'lv', 'tr', 'cy']:
            if lang in labels:
                try:
        ...
        for prop in claims:
            for claim in claims[prop]:
                if claim.type == "wikibase-item":
                    ...
                    pywikibot.output("%s: %s %s" % (self.label(prop), claim.getTarget().amount,
    ...
    def createdateclaim(self, text):
        ...
        if m:
            ...
            day = int(m.group(1))
        m = re.match("(\d+)(?:\.|er|eme|ème)?[\s.]\s*(?:d'|d[aei] )?([^\s.]{2,})\.?[\s.]\s*(\d+)$", text)
        if m:
            year = int(m.group(3))
            try:
                month = self.MONTHNUMBER[m.group(2).lower()]
        ...
        if m:
            ...
            try:
                ...
                raise ValueError("Don't know month {}".format(m.group(2)))
            day = int(m.group(3))
        m = re.match("(\d+) (?:de |d')?(\w+[a-z]\w+) de (\d+)", text)
        if m:
            year = int(m.group(3))
            try:
                month = self.MONTHNUMBER[m.group(2).lower()]
    ...
    def createquantityclaim(self, text):
        m = re.match("(\d+(?:\.\d+)?)\s*([a-z]\w*)", text.replace(',', '.'))
        if not m:
            return None
        amount = m.group(1)
                        ...
                        if mainclaim.type == "external-id" or prop == "P973":
                            ...
                            try:
                                        ...
                                        if claim[0] in updatedclaims and self.isinclaims(claim[1],
                                            ...
                                            if self.QRE.match(claim[1]):
                                                ...
                                                try:
                                                    ...
                                                    print('Unable to analyze date "{}" for {}: {}'.format(
                                                        claim[1][6:], self.label(claim[0]), ex))
                                            ...
                                            try:
                                                ...
                                                if claim[1].startswith("!i!"):
                                                    pywikibot.output('Unable to save image {}: {}'.format(
                                                        claim[1][3:], ex))
                                                    continue
                                                else:
                                                    raise
                                            ...
                                            if claim[2]:
                                                ...
                                                pywikibot.output("Sourcing {}: {}".format(
                                                    self.label(claim[0]), self.label(claim[1])))
                                                try:
                                ...
                                for (language, description) in analyzer.getdescriptions():
                                    newdescriptions[language].add(description.rstrip('.') if
                                                                  len(description) < 250 else description[:246] + "...")
    ...
    def definelabels(self, existinglabels, existingaliases, newnames):
        ...
        for (language, name) in newnames:
            name = name.strip()
            if name.lower() == (existinglabels.get(language) or '').lower() or \
                    name.lower() in [n.lower() for n in existingaliases.get(language, [])]:
                continue
    ...
    def isclaim(self, value, claim):
        try:
            ...
            if str(claim.getTarget()) == value:
                ...
                return True
            elif claim.type == "commonsMedia" and \
                    claim.getTarget().title().split(':', 1)[1].replace('_', ' ') == value.replace('_', ' '):
                return True
            elif claim.type == "time" and \
                    self.showtime(claim.getTarget()) == self.showtime(self.createdateclaim(value)):
                return True
...
class Analyzer:
    ...
    def finddefaultmixedrefs(self, html, includesocial=True):
        ...
        isniresult = re.search('isni\.org/isni/(\d{4})(\d{4})(\d{4})(\w{4})', html)
        if isniresult:
            result.append(("P213", "{} {} {} {}".format(isniresult.group(1), isniresult.group(2), isniresult.group(3), isniresult.group(4))))
        ...
        return [r for r in result if r[1] and
                not (r[0] == 'P2002' and r[1] == 'intent') and
                not (r[0] == 'P2013' and r[1].startswith('pages')) and
                not (r[0] == 'P2013' and r[1] in ['pg', 'plugins', 'sharer']) and
                not (r[0] == 'P214' and r[1].lower() == 'sourceid') and
                not (r[0] == 'P3258' and r[1].lower() in ['users', 'comunity', 'www']) and
                not r[1].lower() == 'search' and
                not (r[0] == 'P3365' and ('(Dizionario_Biografico)' in r[1] or '(Enciclopedia-Italiana)' in r[1] or '(Enciclopedia-dei-Papi)' in r[1])) and
                not (r[0] == 'P2013' and '.php' in r[1])
        ]
...
class IsniAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        return [ self.findbyre('([^\(]*)', name) for name in self.getvalues('Name', html) ]
...
class GndAnalyzer(Analyzer):
    ...
    def findlongtext(self, html):
        return re.sub('\s', ' ', self.findbyre(r'(?s)(<table id="fullRecordTable" .*?</table>)', html) or ''). \
            replace('<tr>', '\n')
...
class LcAuthAnalyzer(Analyzer):
    ...
    def findbirthplace(self, html):
        return self.findbyre(
            '(?s)Birth Place</h3><.*?>(?:\([^<>]*\))?([^<>]+)\s*(?:\([^<>]*\))?\s*</?[au]', html, 'city')

    def finddeathplace(self, html):
        return self.findbyre(
            '(?s)Death Place</h3><.*?>(?:\([^<>]*\))?([^<>]+)\s*(?:\([^<>]*\))?\s*</?[au]', html, 'city')
...
class BnfAnalyzer(Analyzer):
    ...
    def findgender(self, html):
        return self.findbyre(r'(?s)Sexe[^<>]+:.*?<span.*?>(.*?)</', html, 'gender')
    ...
    def findbirthplace(self, html):
        ...
        section = self.findbyre(r'(?s)Naissance.*?(<.*?>)\s*</div>', html)
        if section:
            result = self.findbyre(r',([^<>]+)<', section, 'city')
        if not result:
...
class SbnAnalyzer(Analyzer):
    ...
    def findoccupations(self, html):
        section = self.findbyre(r'(?s)Nota informativa.*?detail_value">([^<>]*?)\.', html)
        if section:
...
class MusicBrainzAnalyzer(Analyzer):
    ...
    def findformationlocation(self, html):
        if not self.isperson:
            return self.findbyre(r'(?s)<dt>Founded in:</dt>.*?<bdi>(\w+)', html, 'city') or \
               self.findbyre(r'(?s)<dt>Founded in:</dt>.*?<bdi>(.*?)</bdi>', html, 'city') or\
               self.findbyre(r'(?s)<dt>Area:</dt>.*?<bdi>(.*?)</bdi>', html, 'city')
...
class StructuraeAnalyzer(Analyzer):
    ...
    def findlanguagenames(self, html):
        return [(m[0], m[1].replace("-", " ")) for m in
                re.findall('(?s)"alternate"[^<>]*hreflang="(\w+)"[^<>]*/([^<>"]*)">', html)]
...
class CbdbAnalyzer(Analyzer):
    ...
    def findnationalities(self, html):
        return [
            self.findbyre(r'(?s)<b>生年</b>:\s*(.)', html, 'dynasty') or
            self.findbyre(r'(?s)<b>生年</b>:\s*(..)', html, 'dynasty'),
            self.findbyre(r'(?s)<b>卒年</b>:\s*(.)', html, 'dynasty') or
...
class NkcrAnalyzer(Analyzer):
    ...
    def findnationality(self, html):
        section = self.getvalue('Související zem.', html) or\
                  self.getvalue('Biogr\./Hist\. .daje', html)
        if section:
...
class IbdbAnalyzer(Analyzer):
    ...
    def findbirthplace(self, html):
        return self.findbyre(
            '(?s)<div class="xt-lable">Born</div>\s*<div class="xt-main-title">'
            '[^<>]*</div>\s*<div class="xt-main-moreinfo">(.*?)</div>',
            html, 'city')
    ...
    def finddeathplace(self, html):
        return self.findbyre(
            '(?s)<div class="xt-lable">Died</div>\s*<div class="xt-main-title">[^<>]*</div>'
            '\s*<div class="xt-main-moreinfo">(.*?)</div>',
            html, 'city')
...
class NndbAnalyzer(Analyzer):
    ...
    def getvalues(self, field, dtype=None, bold=True):
        rawtexts = self.findallbyre(
            '%s%s:%s\s*(.+?)<(?:br|p)>' % ('<b>' if bold else ' ', field, '</b>' if bold else ''), self.html)
...
class ConorAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        return self.findallbyre('<h\d+>(.*?)<', html)
...
class ConorBgAnalyzer(ConorAnalyzer):
    ...
    def findfirstnames(self, html):
        return [self.findbyre(',\s*(\w+)', name, 'firstname') for name in self.findnames(html)[:2]]
...
class ClaraAnalyzer(Analyzer):
    ...
    def findoccupations(self, html):
        ...
        section = self.findbyre(
            '(?s)<div class="detail_label">Artistic Role\(s\):</div>\s*<div class="detail_text">(.*?)<', html)
        if section:
            result = result + self.findallbyre(r'([^,]*)', section, 'occupation')
        section = self.findbyre(
            '(?s)<div class="detail_label">Other Occupation\(s\):</div>\s*<div class="detail_text">(.*?)<', html)
        if section:
            result = result + self.findallbyre(r'([^,]*)', section, 'occupation')
        return result
    ...
    def findresidences(self, html):
        section = self.findbyre(
            '(?s)<div class="detail_label">Place\(s\) of Residence:</div>\s*<div class="detail_text">(.*?)<', html)
        if section:
...
class NlpAnalyzer(Analyzer):
    ...
    def finddeathdate(self, html):
        result = self.findbyre(r' d </TT></I>\([^<>]*-([^<>]*)\)', html)
        if result and 'ca ' not in result: return result
...
class BritishMuseumAnalyzer(Analyzer):
    ...
    def finddetails(self, html):
        return self.findbyre('(?s)Details</dt>\s*<dd[^<>]*>(.*?)</dd>', html) or ''

    def findnames(self, html):
        return self.findallbyre('(?s)name:</span>(.*?)<', html)

    def findlongtext(self, html):
        return self.findbyre('(?s)Biography</dt>\s*<dd[^<>]*>(.*?)</dd>', html)
    ...
    def findbirthdate(self, html):
        return self.findbyre('(?s) dates</dt>\s*<dd[^<>]*>([^<>]*)-', html)
...
class MetallumAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None, alt=None):
        return self.findbyre('(?s)<dt>%s:</dt>\s*<dd[^<>]*>(?:<[^<>]*>)*(.+?)(?:<[^<>]*>)*</dd>'%field, html, dtype, alt=alt)
...
class ResearchGateAnalyzer(Analyzer):
    def setup(self):
        ...
        self.hrtre = '<h2[^<>]*>(?:<[^<>]*>|\s)*About(<.*?<h2'
        self.language = 'en'

    def findinstanceof(self, html):
        return self.findbyre('".type":\s*"(.*?)"', html, 'instanceof')
    ...
    def findemployers(self, html):
        result = self.findallbyre('institution[^"<>]*">(?:<[^<>]*>|\s)*(.*?)<', html, 'employer', alt=['university'])
    ...
    def findoccupations(self, html):
        return self.findallbyre('"jobTitle":\s*"(.*?)"', html, 'occupation') +\
            self.findallbyre('[pP]osition(?:<[^<>]*>\s)*?([^<>]*?)</', html, 'occupation')
...
class SvenskFilmAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        result = [self.findbyre(r'<h1[^<>]*>(.*?)<', html)]
        section = self.findbyre(r'(?s)<th>Alternativnamn</th>\s*<td>(.*?)</td>', html)
        if section:
...
class NgaAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        ...
        if section:
            return [self.findbyre(r'<dt class="artist">(.*?)<', html)] + self.findallbyre(r'(\w.+)', section)
        else:
...
class StuttgartAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('<label[^<>]*>\s*%s\s*<.*?"form_input_element">(.*?)<'%field, html, dtype)

    def getvalues(self, field, html, dtype=None):
        sections = self.findallbyre('<label[^<>]*>\s*%s\s*<.*?"form_input_element">(.*?)<'%field, html)
        result = []
        for section in sections:
    ...
    def findnames(self, html):
        result = self.getvalues('alt. Names', html)
    ...
    def findchildren(self, html):
        result = self.getvalues('Children', html, 'person')

    def findfather(self, html):
        result = self.getvalue("Father's occupation", html, 'person')
    ...
    def findmixedrefs(self, html):
        return [('P214', code) for code in self.getvalues('viaf\d*', html)]
...
class OdisAnalyzer(Analyzer):
    ...
    def findresidences(self, html):
        ...
        if section:
            ...
            for subsection in subsections:
                result.append(
                    self.findbyre(r'<td width="auto">([^<>]*)</td>', subsection, 'city') or
                    self.findbyre(r'<span[^<>]*>(.*?)<', subsection, 'city')
                )
            return result
    ...
    def findemployers(self, html):
        section = self.findbyre(
            '(?s)<b>Engagementen in organisaties en instellingen</b>\s*</td>\s*</tr>\s*<tr>(.*?)</tbody>', html)
        if section:
    ...
    def findsportteams(self, html):
        ...
        if section:
            return self.findallbyre(
                '(?s)<td class="hauptlink no-border-links hide-for-small vereinsname">\s*<[^<>]*>(.*?)<', html,
                'footballteam')
...
class TheatricaliaAnalyzer(Analyzer):
    ...
    def findoccupations(self, html):
        ...
        sections = self.findallbyre(r'(?s)itemprop="performerIn"[^<>]*>(.*?)<', html)
        for section in sections:
...
class CageMatchAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre(
            '(?s)<div class="InformationBoxTitle">%s:</div>\s*<div class="InformationBoxContents">(.*?)</div>' % field,
            html, dtype)
...
class EcarticoAnalyzer(Analyzer):
    ...
    def findlanguagesspoken(self, html):
        return self.findallbyre('<td>Language</td>\s*<td>[^<>]*</td>\s*<td>(.*?)<', html, 'language')

    def findreligions(self, html):
        return self.findallbyre('<td></td>\s*<td>[^<>]*</td>\s*<td>(.*?)<', html, 'language')
...
class BandcampAnalyzer(Analyzer):
    ...
    def findmixedrefs(self, html):
        section = self.findbyre(r'(?s)<ol id="band-links">(.*?)</ol>', html) or\
                  self.findbyre(r'(?s)(<head.*)', html)
        return self.finddefaultmixedrefs(section)
...
class Chess365Analyzer(Analyzer):
    ...
    def findmixedrefs(self, html):
        return [('P1440', self.findbyre('http://ratings.fide.com/card.phtml\?event=(\d+)', html))]
...
class AdultFilmAnalyzer(Analyzer):
    ...
    def findbirthdate(self, html):
        return self.findbyre('Date of Birth: ([\d/\-]+)', html)

    def findfloruitstart(self, html):
        return self.findbyre('Starting Year: (\d+)', html)
...
class CcedAnalyzer(Analyzer):
    ...
    def findschools(self, html):
        ...
        if section:
            return self.findallbyre(
                '(?s)<tr[^<>]*>\s*<td>[^<>]*</td>\s*<td>[^<>]*</td>\s*<td>[^<>]*</td>\s*<td>[^<>]</td>\s*<td>([^<>]*)',
                section, 'university')
    ...
    def finddegrees(self, html):
        ...
        if section:
            return self.findallbyre(
                '(?s)<tr[^<>]*>\s*<td>[^<>]*</td>\s*<td>[^<>]*</td>\s*<td>[^<>]*</td>\s*<td>([^<>]*)', section,
                'degree')
...
class AutoresArAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('>%s:\s*(?:<[^<>]*>)*([^<>]+)<'%field, html, dtype)
    ...
    def finddescriptions(self, html):
        section = self.findbyre('<h3[^<>]*>Disciplina:\s*</h3>(.*?)<hr', html)
        if section:
            return self.findallbyre('>(.+?)<', section)
        else:
            return []
    ...
    def findbirthdate(self, html):
        return self.getvalue('Fecha de nacimiento', html) or\
            self.getvalue('Año de nacimiento', html)

    def finddeathdate(self, html):
        return self.getvalue('Fecha de muerte', html) or\
            self.getvalue('Año de muerte', html)
...
class ItauAnalyzer(Analyzer):
    def setup(self):
        ...
        if self.isperson:
            ...
            self.urlbase = None  # Analyzer only created for persons,
...
class AKLAnalyzer(Analyzer):
    ...
    def findfirstname(self, html):
        return self.findbyre('"pf:contentName"\s*:\s*"[^"]*?,\s*(\w+)', html)

    def findlastname(self, html):
        return self.findbyre('"pf:contentName"\s*:\s*"([^"]*?),', html)

    def findbirthdate(self, html):
        return self.findbyre('<b>Beruf</b>.*?<dd class="fieldValue">([\d\.]+)', html)
...
class SpanishBiographyAnalyzer(Analyzer):
    ...
    def finddeathdate(self, html):
        return self.findbyre(r'"description" content="[^"]+–([^"]*?)\)', html) or \
               self.findbyre(
                   '(?:<span style="font-family:\'Times New Roman\';">|</b>)[^<>]*? – [^<>]*?((?:\d+\.\w+\.)?\d+)',
                   html)
...
class AccademiaCruscaAnalyzer(Analyzer):
    ...
    def findbirthplace(self, html):
        return self.findbyre('(?s)<span class="etichetta">Esistenza</span>\s*<span class="campo">(.*?) [\d—]', html, 'city')

    def findbirthdate(self, html):
        return self.findbyre('(?s)<span class="etichetta">Esistenza</span>\s*<span class="campo">[^<>]*? (\d[\w\d\s]*)—', html)
    ...
    def findoccupations(self, html):
        section = self.findbyre('(?s)<span class="etichetta">Nota biografica</span>\s*<span class="campo">(.*?)<', html)
        if section:
            result = []
...
class GameFaqsAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('(?s)<b>%s:</b>(?:\s|<[^<>]*>)*([^<>]+)'%field, html, dtype)
    ...
    def findgenres(self, html):
        return self.findallbyre('/category/\d+ ([^"<>]+)', html.replace('-', ' '), 'gamegenre')

    def findfranchises(self, html):
        return self.findallbyre('/franchise/\d+ ([^"<>]+)', html.replace('-', ' '), 'franchise')
...
class AinmAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        result = [self.findbyre(r'<meta name="title" content="(.*?)[\(\|"]', html)]
...
class RollDaBeatsAnalyzer(Analyzer):
    ...
    def findgenres(self, html):
        section1 = self.findbyre('(?s)Genre:\s*</span>(.*?)<', html) or ''
        section2 = self.findbyre('(?s)Style:\s*</span>(.*?)<', html) or ''
        return self.findallbyre('([^,]+)', section1, 'muziekgenre', alt=['genre']) +\
            self.findallbyre('([^,]+)', section2, 'muziekgenre', alt=['genre'])

    def findparts(self, html):
        section = self.findbyre('(?s)>Member\(s\)</li>(.*?)</ul>', html) or ''
    ...
    def findresidence(self, html):
        ...
        members = self.findallbyre('>(.*?)</a>', section)
        if len(members) == 1:
            return self.findbyre('</a>\s*<span>\((.*?)\)', html, 'city')
...
class PornhubAnalyzer(Analyzer):
    def setup(self):
        ...
        self.dbid = 'Q936394'
        self.dbname = 'Pornhub'
        self.urlbase = 'https://www.pornhub.com/pornstar/{id}'
        self.hrtre = '<div class="detailedInfo">(.*?)</section>'
        self.language = 'en'
        self.showurl = False

    def getvalue(self, field, html, dtype=None):
        return self.findbyre(r'(?si)itemprop="%s"[^<>]*>(.*?)<'%field, html, dtype) or\
            self.findbyre(r'(?si)"infoPiece"><span>%s:</span>(?:\s|<[^<>]*>)*([^<>]*)'%field, html, dtype)
    ...
    def findbirthdate(self, html):
        return self.getvalue('birth\s*date', html) or self.getvalue('born', html)

    def findbirthplace(self, html):
        return self.getvalue('birth\s*place', html, 'city')

    def finddeathdate(self, html):
        return self.getvalue('death\s*date', html) or self.getvalue('died', html)

    def finddeathplace(self, html):
        return self.getvalue('death\s*place', html, 'city')

    def findheight(self, html):
        section = self.getvalue('height', html)
        if section:
            return self.findbyre('(\d+ cm)', section)

    def findweights(self, html):
        section = self.getvalue('weight', html)
        if section:
            return self.findallbyre('(\d+ \w+)', section)
    ...
    def findeyecolor(self, html):
        return self.getvalue('eye\s*color', html, 'eyecolor')

    def findhaircolor(self, html):
        return self.getvalue('hair\s*color', html, 'haircolor')
...
class YoupornAnalyzer(Analyzer):
    ...
    def findmixedrefs(self, html):
        return finddefaultmixedrefs(html)
...
class NatGeoCanadaAnalyzer(Analyzer):
    ...
    def findnationalities(self, html):
        ...
        if section:
            return self.findallbyre(r'([^,]+)', re.sub('\(.*?\)', ',', section), 'country')
...
class EntomologistAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('<SPAN[^<>]*>%s:\s*</SPAN>(?:\s|<[^<>]*>)*([^<>]*)'%field, html, dtype)
    ...
    def findbirthplace(self, html):
        ...
        if section:
            return self.findbyre('^\s*(?:in ?)(.*)', section, 'city')
...
class Edit16Analyzer(Analyzer):
    ...
    def findnames(self, html):
        ...
        print('section: {}, result: {}'.format(section, result))
    ...
    def findbirthplace(self, html):
        return self.findbyre(' nato a (\w+)', html, 'city')

    def finddeathplace(self, html):
        return self.findbyre(' morto a (\w+)', html, 'city')

    def findbirthdate(self, html):
        return self.findbyre(' nato a[\w\s]* (?:nel|il) ([\w\s]*)', html)
...
class RismAnalyzer(Analyzer):
    ...
    def findbirthdate(self, html):
        if not 'fl.' in (self.getvalue('Schaffensjahre', html) or ''):
...
class RedTubeAnalyzer(Analyzer):
    ...
    def findlongtext(self, html):
        return self.findbyre('(?s)<div class="pornstar_info_bio[^<>]*long_description">(?:\s|<[^<>]*>)*(.*?)<', html) or\
            self.findbyre('(?s)<div class="pornstar_info_bio[^<>]*description">(?:\s|<[^<>]*>)*(.*?)<', html)
    ...
    def findwebsite(self, html):
        return self.findbyre('(?s)href="([^<>"]*)"[^<>]*>(?:\s|<[^<>]*>)*Official Site', html)
...
class PuscAnalyzer(Analyzer):
    ...
    def getcode(self, code, html):
        return self.findbyre(
            '(?s)<b>Source of number or code:</b>\s*%s</p>\s*<p><b>Standard number or code:</b>\s*(.*?)</p>' % code,
            html)
...
class IgdbAnalyzer(Analyzer):
    def setup(self):
        ...
        self.hrtre = '<h3 class="underscratch[^<>]*>(?:<[^<>]*>|\s)*Information(<.*?)<h3 class="underscratch'
        self.language = 'en'
    ...
    def findpubdates(self, html):
        ...
        if section:
            return self.findallbyre('datetime="([\d\-]+)', section)
    ...
    def findengines(self, html):
        return self.getvalues('Game engine', thml, 'engine')
...
class MuziekwebAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        return [self.name] + self.findallbyre('itemprop="\w*[nN]ame">(.*?)<', html)
    ...
    def findinstruments(self, html):
        sections = self.findallbyre(self.name + '\s*<span class="cat-role">\((.*?)\)', html)
        result = []
        for section in sections:
...
class LetterboxdAnalyzer(Analyzer):
    def setup(self):
        ...
        self.urlbase = 'https://letterboxd.com/film/{id}/'
...
class ArticArtistAnalyzer(Analyzer):
    ...
    def findinstanceof(self, html):
        return self.findbyre('<meta name="description"\s*content="(.*?)"', html, 'instanceof')

    def findnames(self, html):
        return (self.findbyre('<dd itemprop="additionalName">(.*?)</dd>', html) or '').split(',') +\
            self.findallbyre('title"\d*content="(.*?)["|]', html)

    def findlongtext(self, html):
        return self.findbyre('(?s)itemprop="description">(.*?)</div>', html)

    def finddescriptions(self, html):
        return self.findallbyre('<meta name="description"\s*content="(.*?)"', html)
...
class AcademieRouenAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('(?s)%s.\s*:\s*</strong>.*?<td[^<>]*>(?:<[^<>]*>)*([^<>]+)<'%field, html, dtype)
...
class MutualAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        ...
        if section:
            result += self.findallbyre('([\w\s]+)', section)
        return result
...
class GuggenheimAnalyzer(Analyzer):
    ...
    def findbirthdate(self, html):
        return self.findbyre('Born: ([\d\-]+)', html)

    def finddeathdate(self, html):
        return self.findbyre('Died: ([\d\-]+)', html)
...
class UvaAlbumAnalyzer(Analyzer):
    ...
    def findfirstname(self, html):
        ...
        return self.findbyre('([\w\-]+)', section, 'firstname')
    ...
    def findbirthdate(self, html):
        ...
        if birthdata:
            return self.findbyre('(.*\d),', birthdata)

    def findbirthplace(self, html):
        birthdata = self.getvalue('Geboren', html)
        if birthdata:
            return self.findbyre('.*\d,(.*)', birthdata, 'city')
    
    def finddeathdate(self, html):
        deathdata = self.getvalue('Overleden', html)
        if deathdata:
            return self.findbyre('(.*\d),', deathdata)

    def finddeathplace(self, html):
        deathdata = self.getvalue('Overleden', html)
        if deathdata:
            return self.findbyre('.*\d,(.*)', deathdata, 'city')
    ...
    def findschools(self, html):
        return self.getvalues('Instelling \(opleiding\)', html, 'university')

    def findadvisors(self, html):
        return self.getvalues('Promotor\(en\)', html, 'scientist')
    ...
    def findemployers(self, html):
        return self.getvalues('Instelling \(aanstelling\)', html, 'employer', alt=['university'])
...
class AlvinAnalyzer(Analyzer):
    ...
    def finddeathdate(self, html):
        return self.findbyre('Death (\d+)', html)
    ...
    def findgender(self, html):
        return self.findbyre('\(Person\)(?:\s|<[^<>]*>)*<div class="singleRow">(.*?)<', html, 'gender')
...
class PlwabnAnalyzer(Analyzer):
    ...
    def getvalue(self, field, letter, html, dtype=None):
        row = self.findbyre('(<tr><td[^<>]*>%s\s*<.*?</tr>)'%field, html)
        if row:
            return self.findbyre('<I>\s*%s\s*</TT></I>(.*?)<'%letter, row, dtype)

    def getvalues(self, field, letter, html, dtype=None):
        result = []
        rows = self.findallbyre('(<tr><td[^<>]*>%s\s*<.*?</tr>)'%field, html)
        for row in rows:
            result += self.findallbyre('<I>\s*%s\s*</TT></I>(.*?)<'%letter, row, dtype)
        return result
    ...
    def findsources(self, html):
        ...
        for source in sources:
            if source and not ' by ' in source and not ' / ' in source:
...
class BewebAnalyzer(Analyzer):
    ...
    def findlanguagenames(self, html):
        ...
        if section:
            result += [('it', name) for name in self.findallbyre('(?s)<li>\s*(.*?)[&<]', section)]
        section = self.findbyre('(?s)Forme varianti</h3>(.*?</ul>)', html)
        if section:
            result += [(self.languagetranslate.get(lang, lang[:2]), name) for (name, lang) in re.findall('([^<>\(\)&]*)\(([^<>\(\)&]*)\)', section)]
            result += [('it', name) for name in self.findallbyre('([^<>\(\)&]*?)[<&]', section)]
        else:
    ...
    def findmixedrefs(self, html):
        ...
        sbn = self.findbyre('<span id="codSBN">[^<>]*</span>[^<>]*?([^\s]*)<', html)
        if sbn:
            result += [('P396', sbn)]
        return result

    def findviaf(self, html):
        return self.findbyre('Codice VIAF[^<>]*?(\d+)<', html)

    def findisni(self, html):
        return self.findbyre('Codice ISNI[^<>]*?(\d\w*)<', html)
    ...
    def findwebpages(self, html):
        ...
        for text in ['wikipedia', 'id.loc.gov', 'd-nb.info', 'bnf.fr', 'getty.edu', 'viaf.org',
            ...
            links = [l for l in links if not text in l]
        return links
...
class WorldsWithoutEndAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('(?s)>%s:</td>\s*<td[^<>]*>(.*?)</td>'%field, html, dtype)
...
class BelgianPhotographerAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None, alt=[]):
        return self.findbyre('(?s)<h3>\s*%s\s*</h3>\s*<div[^<>]*>(.*?)</div>'%field, html, dtype, alt=alt)

    def getvalues(self, field, html, dtype=None, alt=[]):
    ...
    def findnames(self, html):
        return self.findallbyre('<title>([^<>]*) - ', html) +\
            ...
            self.getvalues('Standardized form\(s\) of name according to other rules', html)

    def findlongtext(self, html):
        return self.TAGRE.sub(' ', self.getvalue('Activity', html) or '')

    def findgender(self, html):
        return self.findbyre('Person \((.*?)\)', html, 'gender')
    ...
    def finddeathdate(self, html):
        section = self.getvalue('Life dates', html)
        if section:
            return self.findbyre(', \d+ - [\w\s]+, (\d{4})', section)
        
    def findbirthplace(self, html):
        section = self.getvalue('Life dates', html)
        if section:
            return self.findbyre('(.*), \d+ - ', section, 'city')

    def finddeathplace(self, html):
        section = self.getvalue('Life dates', html)
        if section:
            return self.findbyre(', \d+ - ([^<>]+), \d{4}', section, 'city')

    def findworkplaces(self, html):
        return [self.findbyre('[\d\s\-\+]+(.*)', section) for section in self.getvalues('Locations', html)]
...
class AlkindiAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('(?s)<p class="ltr (?:notice-label|text-muted)">\s*%s.*?<[^<>]* class="ltr"\s*>(.*?)<'%field, html, dtype)

    def getvalues(self, field, html, dtype=None):
        return self.findallbyre('(?s)<p class="ltr (?:notice-label|text-muted)">\s*%s.*?<[^<>]* class="ltr"\s*>(.*?)<'%field, html, dtype)

    def instanceof(self, html):
        return 'Q5'

    def findnames(self, html):
        return self.findallbyre('(?s)data-original-title="[^"]*ccess point">(.*?)(?:,[\s\d‒]*)?<', html.replace('،', ','))
...
class ZobodatAnalyzer(Analyzer):
    def setup(self):
        ...
        self.skipfirst = True
        self.hrtre = "<div class='detail-container clearfix content-box\s*'>(.*?)<div id='footer'>"
        self.language = 'de'
    ...
    def finddescription(self, html):
        return self.findbyre('(?s)</h1>(?:\s|<[^<>]*>)*([^<>]+)', html)

    def findlongtext(self, html):
        result = self.findbyre('(?s)</h1>\s*<p>(.*?)</p>', html)
        if result:
            return self.TAGRE.sub(' ', result)

    def findbirthdate(self, html):
        return self.findbyre('>\*\s*([^\s<>]+)', html)

    def findbirthplace(self, html):
        return self.findbyre('>\*\s*[^\s<>]+\s([^<>]+)', html, 'city')
    ...
    def findwebpages(self, html):
        section = self.findbyre('</h1>\s*<p>(.*?)</p>', html)
        if section:
...
class PatrinumAnalyzer(Analyzer):
    def setup(self):
        ...
        self.hrtre = 'Notice détaillée\s*</h3>(.*?)(?:<h3|<!--)'
        self.language = 'fr'

    def getvalue(self, field, html, dtype=None):
        return self.findbyre("(?s)<div class='metadata-row'><span [^<>]*>\s*%s\s*</span>\s*<span [^<>]*>(.*?)</span>"%field, html, dtype)
    ...
    def findlongtext(self, html):
        return self.findbyre('(?s)Historique\s*</h3>(.*?)(?:<h3|<!--)', html)
    ...
    def findwebpages(self, html):
        ...
        if section:
            return self.findallbyre('"(\w+://[^"<>]*)"', section)
...
class JwaAnalyzer(Analyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        result = self.findbyre('(?s)<div class="field-label">\s*%s\s*<.*?<div class=[^<>]*field-item[^<>]*>(.*?)</div>'%field, html)
        if result:
            result = self.TAGRE.sub("", result)
            if dtype:
...
class WikiAnalyzer(Analyzer):
    ...
    def findnames(self, html):
        ...
        sections = self.getinfos(
            ['n[aou]me[\w\s_]*', 'naam\d*', 'name[ns]', 'imię[\w\s]*', 'nimi', 'անուն',
             'pseudonie?m', 'imiona', 'имя(?:[\s_][\w\s_])*', 'под именем(?:_\d+)?', 'име(?:-[\w\s]+)?',
             '人名', '全名', "ім'я(?:[\s_][\w\s_]*)?", 'tên[\w\s]*', '名称', 'імен[аі]', 'псевдонім',
             'beter-bekend-als', 'nombre[\w\s]*', 'الاسم', '[\w\s]*jméno', 'ime', 'nom(?:[\s_][\w\s_]+)?',
             'vardas', 'ook bekend als', 'alias', 'otros nombres', 'שם[\w\s]*', 'név', 'ふりがな',
             '別名義', "ім'я[\w\s\_]*", 'псевдонім', 'прізвисько', 'pseudônimos?', 'όνομα', 'есімі',
             'isim', 'isimleri', 'adı', 'نام', 'لقب', 'imię[\w\s]*', 'alcume', '芸名', '本名', '이름',
             '본명', 'anarana', 'jina[\w\s]*', 'nom de naissance', '姓名', '原名', 'navn', 'nazwa',
             'títol original', 'přezdívka', 'pseŭdonomo', 'nomo', 'alias', 'namn', 'pseudonym(?:\(er\))?',
             'názov[\w\s]*', 'názvy', '名前', 'jinak zvaný', 'есімі', 'doğum_adı',
            ], html)
        for section in sections:
            result += self.findallbyre(r'([^,;]*)', section)
        return [
    ...
    def excludetemplate(text):
        ...
        return templatetype in ['sourcetext', 's-bef', 's-ttl', 's-aft', 'appendix', 'familytree', 'ref-llibre',
               ...
               firstword in ['citeer', 'cite', 'ouvrage', 'link', 'grafikus', 'cita', 'cytuj', 'книга', 'citar',
                             'ouvrage', 'citation', 'erreferentzia', 'lien', 'citace', 'citácia',] or \
               lastword in ['source', 'स्रोत', '인용'] or \
    ...
    def findoccupations(self, html):
        return self.getinfos(
            ['charte', 'attività\s*(?:altre)?\d*', 'occupation', 'zawód', 'functie', 'spfunc', 'beroep',
             ...
             'asa', 'kazi yake', '(?:antaŭ|aliaj)?okupoj?\d*', 'работил', 'ocupacio', 'aktywność zawodowa',
             'funkcja', 'profesio', 'ocupație', 'povolání', 'töökoht', 'szakma', 'profession',
             ], html, 'occupation') + \
               self.findallbyre(r'(?i)info(?:box|boks|taula|kast)(?:\s*-\s*)?([\w\s]+)', html, 'occupation') + self.findallbyre(
            '基礎情報([\w\s]+)', html, 'occupation') + \
               self.findallbyre(r'{([\w\s]+)infobox', html, 'occupation') + self.findallbyre(
            'Categorie:\s*(\w+) (?:van|der) ', html, 'occupation') + \
               self.findallbyre(r'(?i)inligtingskas([\w\s]+)', html, 'occupation')

    def findpositions(self, html):
        return self.getinfos(
            ['functie\d?', 'titre\d', 'stanowisko\d*', '(?:\d+\. )?funkcja', 'должность(?:_\d+)?', 'títulos?\d*',
    ...
    def findspouses(self, html):
        return self.getinfos(
            ['spouse', 'consorte', 'conjoint', 'małżeństwo', 'mąż', 'супруга', 'съпруга на', '[\w\s]*брак',
    ...
    def findchildren(self, html):
        return self.getinfos(
            ['issue', 'figli', 'enfants', 'children', 'kinder(?:en|s)', '(?:\d+\. )?dzieci', 'дети', 'потомство',
    ...
    def findbirthdate(self, html):
        return self.findbyre(
            '{{(?:[bB]irth[\-\s]date(?: and age)?|dni|[dD]oğum tarihi ve yaşı|출생일(?:과 나이)?|[gG]eboortedatum(?: en ouderdom)?|'
            'Data naixement|[dD]atum narození a věk|Naskiĝdato|[dD]atum rođenja|生年月日と年齢|死亡年月日と没年齢|'
            'роден на|[dD]ate de naissance|'
            ')\s*(?:\|df=\w+)?\|(\d+\|\d+\|\d+)', html) or \
            self.findbyre('{{[dD]ate de naissance\|([\w\s]+)\}\}', html) or\
            self.findbyre('{{(?:[bB]irth year and age)\|(\d+)', html) or\
            ...
            'Categorie:\s*Nașteri în (.*?)\]', html) or \
               self.findbyre(r'(.*)-', self.getinfo(['leven'], html) or '') or self.findbyre(
            'Kategory:Persoan berne yn(.*?)\]', html) or \
        ...
        return self.findbyre(r'{{(?:[dD]eath (?:date|year)[\w\s]*|morte|사망일과 나이|Data defunció i edat|[dD]atum smrti i godine|'
               ...
               self.getinfo(
                   ['sterbedatum', 'death[_ ]?date', 'data di morte', 'annomorte', 'date de décès', 'gestorven', 'død',
                    'data śmierci', 'data_defuncio', 'sterf(?:te)?dat\w*', 'data_morte', 'дата смерти', '死亡日',
    ...
    def findbirthplace(self, html):
        return self.getinfo(
            ['birth[_ ]?place', 'luogo di nascita', 'luogonascita\w*', 'geboren_in', 'geburtsort', 'fødested',
             'geboorteplaats', 'miejsce urodzenia', 'lloc_naixement', 'gbplaats', 'место рождения', 'място на раждане',
             '生地', 'місце_народження', 'lugar\s*de\s*nac[ei]mi?ento', 'مكان الولادة', 'lieu de naissance',
             ...
             'τόπος γέννησης', 'туған жері', 'mj?esto[\s_]rođenja', 'doğum_yeri', 'محل تولد', 'local_nascimento',
    ...
    def finddeathplace(self, html):
        return self.getinfo(
            ['death[_ ]?place', 'luogo di morte', 'luogomorte', 'lieu de décès', 'gestorven_in', 'sterbeort',
             ...
             'τόπος θανάτου', 'қайтыс болған жері', 'mj?esto_[\s_]mrti', 'ölüm_yeri', 'محل مرگ', 'починал-място',
    ...
    def findnationalities(self, html):
        return self.getinfos(
            ['nazionalità[\w\s_]*', 'allégeance', 'land', 'nationality', 'narodowość', 'państwo', 'громадянство',
             ...
             'nationalteam\d*', 'ŝtato', '國家', 'občanství', 'kodakondsus', 'rahvus', 'kansalaisuus',
    ...
    def findfirstname(self, html):
        return self.getinfo(['antaŭnomo', 'nome', 'meno', 'firstname'], html, 'firstname') or \
    ...
    def findschools(self, html):
        return self.getinfos(['education', 'alma[ _]?m[aá]ter', 'edukacja', '[\w\s]*uczelnia', 'formation', 'skool',
    ...
    def findemployers(self, html):
        return self.getinfos(['employer', 'pracodawca', 'institutions', 'empleador', 'jednostka podrz\d* nazwa',
    ...
    def findawards(self, html):
        ...
        return [self.findbyre('([^\(\)]+)', r, 'award') for r in result]
    ...
    def findgenres(self, html):
        return self.getinfos(['stijl', 'g[eè]ne?re\d*', 'estilo', 'ジャンル', 'жанр(?:ове)?', 'tyylilajit',
                              'ժանրեր', 'sjanger', 'xénero', 'genre\(r\)', 'gatunek', 'žáne?r',
    ...
    def findnotableworks(self, html):
        return self.getinfos(['notable[\s_]?works?', 'bekende-werken', '\w+ notables', '主な作品',
    ...
    def findworkfields(self, html):
        return self.getinfos(['field', '(?:main_)?interests', 'known_for', 'زمینه فعالیت', 'specjalność',
                              '[\w\s_]*dyscyplina', 'área', 'научная сфера', 'domaine', 'known_for',
    ...
    def findsportteams(self, html):
        return self.getinfos(['clubs?\d*', '[\w\s]*kluby', 'klub aktuální\s*\w?', 'tým'], html, 'club')
    ...
    def findpseudonyms(self, html):
        return self.getinfos(['псевдонім', 'pseudon[iy]e?m(?:\(er\))?', 'psudônimos?', 'pseŭdonomo'], html)
    ...
    def findparts(self, html):
        return self.getinfos(['members', 'miembros', 'jäsenet', 'անդամներ', 'integrantes', 'medlemm[ae]r',
                              'membros', 'gründer\d*a?', 'besetzung\d*a?', 'ehemalige\d*a?',
                              'membres(?: actuels)?', 'członkowie', 'membri', 'состав', 'členovia[\w\s]*',
                              'メンバー',
                             ], html, 'musician')
    ...
    def findinfluences(self, html):
        return self.getinfo(['influenced by'], html)
...
class BibliotecaNacionalAnalyzer(UrlAnalyzer):
    ...
    def findnames(self, html):
        section = self.getvalue('Nombre personal', html)
        if section:
            section = self.findbyre('(?s)>([^<>]*\w[^<>]*)<', section)
...
class OnstageAnalyzer(UrlAnalyzer):
    ...
    def findnames(self, html):
        return [self.findbyre('<h1[^<>]*>(?:<[^<>]*>|\s)*(.*?)</', html)]

    def findbirthdate(self, html):
        return self.findbyre('\(([^<>\(\)]*)-[^<>\(\)]*\)\s*</h1>', html)

    def finddeathdate(self, html):
        return self.findbyre('\((?!\s*fl\.)[^<>\(\)]-([^<>\(\)]*)\)\s*</h1>', html)

    def findmixedrefs(self, html):
        return self.finddefaultmixedrefs(html)

    def findfloruit(self, html):
        return self.findbyre('\(fl\.([^<>\(\)]*)\)\s*</h1>', html)
...
class IasAnalyzer(UrlAnalyzer):
    ...
    def getsubvalues(self, field, secondfield, html, dtype=None, alt=[]):
        section = self.findbyre(r'(?s)<h3[^<>]*>\s*%s\s*</h3>(.*?)(?:<h3|<div class="scholar__)'%field, html)
        if section:
    ...
    def findschools(self, html):
        ...
        return [self.findbyre('(.*[^\.\s])', result, 'university') for result in results]
...
class FotomuseumAnalyzer(UrlAnalyzer):
    ...
    def getvalue(self, field, html, dtype=None):
        return self.findbyre('(?s)<h3>%s</h3>\s*<div>(.*?)</div>'%field, html, dtype)
    ...
    def findnames(self, html):
        ...
        result.append(self.getvalue('Standardized form\(s\) of name according to other rules', html))
    ...
    def findbirthdate(self, html):
        section = self.getvalue('Life dates', html)
        if section:
            return self.findbyre('(\d{4}) - ', section)

    def finddeathdate(self, html):
        section = self.getvalue('Life dates', html)
        if section:
            return self.findbyre('(\d{4})\s*$', section)
        
    def findbirthplace(self, html):
        section = self.getvalue('Life dates', html)
        if section:
            return self.findbyre('(.*?), \d{4} - ', section, 'city')

    def finddeathplace(self, html):
        section = self.getvalue('Life dates', html)
        if section:
            return self.findbyre(', \d{4} - (.*), \d{4}', section, 'city')
...
class UnivieAnalyzer(UrlAnalyzer):
    ...
    def findnationalities(self, html):
        section = self.findbyre(r'(?s)<div class="artist-information-label">Nationality:</div>'
                                '\s*<div class="artist-information-text">(.*?)</div>', html)
        if section:
            return self.findallbyre(r'([\w\s\-]+)', section, 'country')
...
class WeberAnalyzer(UrlAnalyzer):
    ...
    def finddeathplace(self, html):
        return self.findbyre(
            '(?s)<strong>†</strong>\s*</div>\s*<div[^<>]*>\s*<span>[^<>]*</span>\s*<span>\s*(?:in )([^<>]*)<',
            html, 'city')
...
class BacklinkAnalyzer(Analyzer):
    ...
    def findlongtext(self, html):
        matches = re.findall('statement/([qQ]\d+)[^{}]+statement/([pP]\d+)', html)
        return "\n".join("%s of: %s" % (self.bot.label(m[1]), self.bot.label(m[0])) for m in matches)
...
if __name__ == '__main__':
    ...
    if unknownarguments:
        print("Unknown argument(s) found: %s"%(', '.join(unknownarguments)))
    elif item is None:
        print("No item page specified")
    else:
